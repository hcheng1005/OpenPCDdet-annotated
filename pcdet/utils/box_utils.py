...
def boxes_iou_normal(boxes_a, boxes_b):
    ...
    # boxes_a[:, 0, None] shape (74000, 1)
    # boxes_b[None, :, 0] shape (1, num_per_class_gt)
    # torch.max(tensor1, tensor2) element wise  哪个大返回哪个
...
def boxes3d_lidar_to_aligned_bev_boxes(boxes3d):
    ...
    # 如果rot_angle小于45度选择[dx, dy] 如果rot_angle大于45度选择[dy, dx]
    choose_dims = torch.where(rot_angle[:, None] < np.pi / 4, boxes3d[:, [3, 4]], boxes3d[:, [4, 3]])
    """
        对于小于45度的rot_angle  用[l, w]  使用 x - l/2   y - w/2 
        对于大于45度的rot_angle  用[l, w]  使用  x - w/2   y - l/2 
    """
