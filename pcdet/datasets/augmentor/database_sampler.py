...
class DataBaseSampler(object):
    ...
    def filter_by_difficulty(self, db_infos, removed_difficulty):
        ...
        for key, dinfos in db_infos.items():
            ...
            new_db_infos[key] = [
                ...
                if info['difficulty'] not in removed_difficulty  # 逐类别过滤
            ]
            # 记录过滤后的不同类别的infos长度
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if self.sampler_cfg.get('USE_ROAD_PLANE', False):  # 将物体点云加入点云列表
                # mv height
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:  # 如果采样box的数量大于0
                # 在db_infos[calss_name]中随机采样sample_num个box信息
                sampled_dict = self.sample_with_fixed_number(class_name, sample_group)
                # 将采样box的box3d_lidar信息拼接
                sampled_boxes = np.stack([x['box3d_lidar'] for x in sampled_dict], axis=0).astype(np.float32)

                if self.sampler_cfg.get('DATABASE_WITH_FAKELIDAR', False):
                    sampled_boxes = box_utils.boxes3d_kitti_fakelidar_to_lidar(sampled_boxes)

                # 计算采样box和已经存在的box的iou3d
                ...
                # 将有效采样box和已经存在的box进行拼接
                existed_boxes = np.concatenate((existed_boxes, valid_sampled_boxes), axis=0)
                # 将有效的sampled_dict追加到总dict
